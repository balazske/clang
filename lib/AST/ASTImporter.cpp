...
namespace clang {
  ...
  std::string ImportError::getMessage() const {
    switch (Error) {
    case NameConflict:
      return "NameConflict";
...
Error ASTNodeImporter::ImportDeclContext(
  ...
  for (Decl *From : FromDC->decls())
    if (ExpectedDecl ImportedOrErr = import(From))
...
bool ASTNodeImporter::IsStructuralMatch(Decl *From, Decl *To, bool Complain) {
  StructuralEquivalenceContext Ctx(
      ...
      false, Complain);
  bool Result =  Ctx.IsEquivalent(From, To);
  Importer.addStructEqFailsWithDiffCanDecl(
      Ctx.NumSameCanDecl1ComparedWithDifferentCanDecl2);
  return Result;
}
...
bool ASTNodeImporter::IsStructuralMatch(RecordDecl *FromRecord, 
  ...
  StructuralEquivalenceContext Ctx(Importer.getFromContext(),
                                   ...
                                   false, Complain);
  bool Result =  Ctx.IsEquivalent(FromRecord, ToRecord);
  Importer.addStructEqFailsWithDiffCanDecl(
      Ctx.NumSameCanDecl1ComparedWithDifferentCanDecl2);
  return Result;
}
...
bool ASTNodeImporter::IsStructuralMatch(VarDecl *FromVar, VarDecl *ToVar,
  ...
  StructuralEquivalenceContext Ctx(
      ...
      Importer.getNonEquivalentDecls(), getStructuralEquivalenceKind(Importer),
      false, Complain);
  bool Result =  Ctx.IsEquivalent(FromVar, ToVar);
  Importer.addStructEqFailsWithDiffCanDecl(
      Ctx.NumSameCanDecl1ComparedWithDifferentCanDecl2);
  return Result;
}
...
bool ASTNodeImporter::IsStructuralMatch(EnumDecl *FromEnum, EnumDecl *ToEnum,
  ...
  StructuralEquivalenceContext Ctx(
      ...
      Importer.getNonEquivalentDecls(), getStructuralEquivalenceKind(Importer),
      false, Complain);
  bool Result =  Ctx.IsEquivalent(FromEnum, ToEnum);
  Importer.addStructEqFailsWithDiffCanDecl(
      Ctx.NumSameCanDecl1ComparedWithDifferentCanDecl2);
  return Result;
}
...
ExpectedDecl ASTNodeImporter::VisitNamespaceDecl(NamespaceDecl *D) {
  ...
  ExpectedSLoc LocStartOrErr = import(D->getLocStart());
  if (!LocStartOrErr)
...
ASTNodeImporter::VisitTypedefNameDecl(TypedefNameDecl *D, bool IsAlias) {
  ...
  if (auto Imp = importSeq(
      D->getUnderlyingType(), D->getTypeSourceInfo(), D->getLocStart()))
    std::tie(ToUnderlyingType, ToTypeSourceInfo, ToLocStart) = *Imp;
  else
    return Imp.takeError();
  ...
  if (IsAlias) {
    if (GetImportedOrCreateDecl<TypeAliasDecl>(
        ToTypedef, D, Importer.getToContext(), DC, ToLocStart, Loc,
...
ExpectedDecl ASTNodeImporter::VisitEnumDecl(EnumDecl *D) {
  ...
  if (auto Imp = importSeq(
      D->getLocStart(), D->getQualifierLoc(), D->getIntegerType()))
    std::tie(ToLocStart, ToQualifierLoc, ToIntegerType) = *Imp;
  else
    return Imp.takeError();
  ...
  EnumDecl *D2;
  if (GetImportedOrCreateDecl(
          D2, D, Importer.getToContext(), DC, ToLocStart,
...
ExpectedDecl ASTNodeImporter::VisitFunctionDecl(FunctionDecl *D) {
  ...
  if (auto Imp = importSeq(
      ...
      D->getQualifierLoc()))
    std::tie(T, TInfo, ToInnerLocStart, ToQualifierLoc) = *Imp;
  else
    return Imp.takeError();
...
ExpectedDecl ASTNodeImporter::VisitObjCMethodDecl(ObjCMethodDecl *D) {
  ...
  SourceLocation ToLocEnd;
  QualType ToReturnType;
  TypeSourceInfo *ToReturnTypeSourceInfo;
  if (auto Imp = importSeq(
      D->getLocEnd(), D->getReturnType(), D->getReturnTypeSourceInfo()))
    std::tie(ToLocEnd, ToReturnType, ToReturnTypeSourceInfo) = *Imp;
  else
    return Imp.takeError();
  ...
  if (GetImportedOrCreateDecl(
          ...
          ToLocEnd, Name.getObjCSelector(), ToReturnType,
...
ExpectedDecl ASTNodeImporter::VisitObjCInterfaceDecl(ObjCInterfaceDecl *D) {
  ...
  if (!ToIface) {
    ExpectedSLoc AtStartLocOrErr = import(D->getAtStartLoc());
    if (!AtStartLocOrErr)
      return AtStartLocOrErr.takeError();
...
ASTNodeImporter::VisitObjCPropertyImplDecl(ObjCPropertyImplDecl *D) {
  ...
  ObjCImplDecl *InImpl = cast<ObjCImplDecl>(LexicalDC);
  ...
  if (!ToImpl) {
    SourceLocation ToLocStart, ToLocation, ToPropertyIvarDeclLoc;
    if (auto Imp = importSeq(
        D->getLocStart(), D->getLocation(), D->getPropertyIvarDeclLoc()))
      std::tie(ToLocStart, ToLocation, ToPropertyIvarDeclLoc) = *Imp;
    else
      return Imp.takeError();
...
ExpectedDecl ASTNodeImporter::VisitClassTemplateSpecializationDecl(
  ...
  if (FoundDef) {
    ...
    // Import the location of this declaration.
    ExpectedSLoc StartLocOrErr = import(D->getLocStart());
    if (!StartLocOrErr)
      return StartLocOrErr.takeError();
...
ExpectedStmt ASTNodeImporter::VisitDeclStmt(DeclStmt *S) {
  auto Imp = importSeq(S->getDeclGroup(), S->getStartLoc(), S->getEndLoc());
  if (!Imp)
    return Imp.takeError();

  DeclGroupRef ToDG;
  SourceLocation ToStartLoc, ToEndLoc;
  std::tie(ToDG, ToStartLoc, ToEndLoc) = *Imp;
...
ExpectedStmt ASTNodeImporter::VisitCaseStmt(CaseStmt *S) {
  ...
  CaseStmt *ToStmt = new (Importer.getToContext()) CaseStmt(
...
ExpectedStmt ASTNodeImporter::VisitCXXForRangeStmt(CXXForRangeStmt *S) {
  auto Imp1 = importSeq(
      S->getRangeStmt(), S->getBeginStmt(), S->getEndStmt(), S->getCond(),
      S->getInc(), S->getLoopVarStmt(), S->getBody());
  if (!Imp1)
  ...
  Stmt *ToBody;
  std::tie(
      ToRangeStmt, ToBeginStmt, ToEndStmt, ToCond, ToInc, ToLoopVarStmt,
      ToBody) = *Imp1;
  ...
  return new (Importer.getToContext()) CXXForRangeStmt(
      ToRangeStmt, ToBeginStmt, ToEndStmt, ToCond, ToInc, ToLoopVarStmt, ToBody,
...
Expected<Decl *> ASTImporter::Import(const Decl *FromD) {
...
Expected<Decl *> ASTImporter::Import(Decl *FromD) {
  ...
  if (Decl *ToD = GetAlreadyImportedOrNull(FromD)) {
...
Expected<DeclContext *> ASTImporter::ImportContext(DeclContext *FromDC) {
  ...
  ExpectedDecl ToOrErr = Import(cast<Decl>(FromDC));
  if (!ToOrErr)
    return ToOrErr.takeError();

  auto *ToDC = cast<DeclContext>(*ToOrErr);
...
Expected<Expr *> ASTImporter::Import(Expr *FromE) {
  ...
  if (Expected<Stmt *> ToOrErr = Import(cast<Stmt>(FromE)))
    return cast_or_null<Expr>(*ToOrErr);
  else
    return ToOrErr.takeError();
}
...
ASTImporter::Import(NestedNameSpecifier *FromNNS) {
  ...
  auto PrefixOrErr = Import(FromNNS->getPrefix());
  if (!PrefixOrErr)
  ...
  case NestedNameSpecifier::Identifier:
    return NestedNameSpecifier::Create(
        ToContext, *PrefixOrErr, Import(FromNNS->getAsIdentifier()));

  case NestedNameSpecifier::Namespace:
    if (Expected<Decl *> NSOrErr = Import(FromNNS->getAsNamespace()))
      return NestedNameSpecifier::Create(
        ToContext, *PrefixOrErr, cast<NamespaceDecl>(*NSOrErr));
    else
      return NSOrErr.takeError();

  case NestedNameSpecifier::NamespaceAlias:
    if (Expected<Decl *> NSADOrErr = Import(FromNNS->getAsNamespaceAlias()))
      return NestedNameSpecifier::Create(
          ToContext, *PrefixOrErr, cast<NamespaceAliasDecl>(*NSADOrErr));
    else
  ...
  case NestedNameSpecifier::Super:
    if (Expected<Decl *> RDOrErr = Import(FromNNS->getAsRecordDecl()))
      return NestedNameSpecifier::SuperSpecifier(
          ToContext, cast<CXXRecordDecl>(*RDOrErr));
    else
      return RDOrErr.takeError();

  case NestedNameSpecifier::TypeSpec:
  case NestedNameSpecifier::TypeSpecWithTemplate:
    if (Expected<QualType> TyOrErr = Import(
        QualType(FromNNS->getAsType(), 0u))) {
...
Expected<TemplateName> ASTImporter::Import(TemplateName From) {
  ...
  case TemplateName::Template:
    if (Expected<Decl *> ToTemplateOrErr = Import(From.getAsTemplateDecl()))
  ...
  case TemplateName::OverloadedTemplate: {
    ...
    for (OverloadedTemplateStorage::iterator I = FromStorage->begin(),
                                             E = FromStorage->end();
         I != E; ++I) {
      if (Expected<Decl *> ToOrErr = Import(*I))
  ...
  case TemplateName::DependentTemplate: {
    ...
    return ToContext.getDependentTemplateName(
        *QualifierOrErr, DTN->getOperator());
  }

  case TemplateName::SubstTemplateTemplateParm: {
    SubstTemplateTemplateParmStorage *subst
      = From.getAsSubstTemplateTemplateParm();
    Expected<Decl *> ParamOrErr = Import(subst->getParameter());
    if (!ParamOrErr)
  ...
  case TemplateName::SubstTemplateTemplateParmPack: {
    ...
    Expected<Decl *> ParamOrErr = Import(SubstPack->getParameterPack());
    if (!ParamOrErr)
      return ParamOrErr.takeError();
    
    ASTNodeImporter Importer(*this);
    auto ArgPackOrErr = Importer.ImportTemplateArgument(
...
Expected<CXXCtorInitializer *> ASTImporter::Import(CXXCtorInitializer *From) {
  Expected<Expr *> ToExprOrErr = Import(From->getInit());
  if (!ToExprOrErr)
    return ToExprOrErr.takeError();
  ...
  if (From->isBaseInitializer()) {
    ...
    Expected<Decl *> ToFieldOrErr = Import(From->getMember());
    if (!ToFieldOrErr)
      return ToFieldOrErr.takeError();
    ...
    Expected<Decl *> ToIFieldOrErr = Import(From->getIndirectMember());
    if (!ToIFieldOrErr)
...
ASTImporter::Import(const CXXBaseSpecifier *BaseSpec) {
  ...
  if (!ImportedSourceRangeOrErr)
    return ImportedSourceRangeOrErr.takeError();
...
Expected<DeclarationName> ASTImporter::Import(DeclarationName FromName) {
  ...
  case DeclarationName::CXXDeductionGuideName: {
    if (auto TemplateOrErr = Import(FromName.getCXXDeductionGuideTemplate()))
      return ToContext.DeclarationNames.getCXXDeductionGuideName(
          cast<TemplateDecl>(*TemplateOrErr));
    else
